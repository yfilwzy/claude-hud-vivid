...
import { renderIdentityLine, renderProjectLine, renderWorkdirLine, renderEnvironmentLine, renderUsageLine, } from './lines/index.js';
...
function renderExpanded(ctx) {
    ...
    for (let index = 0; index < elementOrder.length; index += 1) {
        ...
        // Insert dedicated workdir line right after project line
        if (element === 'project') {
            const workdirLine = renderWorkdirLine(ctx);
            if (workdirLine) {
                lines.push({ line: workdirLine, isActivity: false });
            }
        }
    }
    return lines;
